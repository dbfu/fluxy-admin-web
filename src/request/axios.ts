...
class Request {
  ...
  private async refreshToken() {
    ...
    auth_refreshToken({ refreshToken: refreshToken.toString() })

    const [error, data] = await to(auth_refreshToken({ refreshToken: refreshToken.toString() }));
    if (error) {
      this.toLoginPage();
      return;
    }
  ...
  private async responseErrorInterceptor(error: any): Promise<any> {
    ...
    if (status === 401) {
      ...
      antdUtils.notification.error({
        message: '出错了',
        description: error?.response?.data?.message || '发生未知错误，请刷新后重试',
      });
...
const request = new Request({ timeout: 60 * 1000 * 5, baseURL: '/api' });
